...
def demo_request():
    ...
    if not demo_email_ready():
        raise ValidationError(
            "Demo requests are temporarily unavailable. Email hello@answerdeck.com and we will get back to you."
        )
    ...
    try:
        ...
        raise ValidationError(
            "We could not send your request right now. Try again shortly or email hello@answerdeck.com."
        ) from exc

    return jsonify(
        {
            "message": "Thanks — your demo request was sent. We will reply within one business day.",
            "sent_to": "hello@answerdeck.com",
        }
    )
